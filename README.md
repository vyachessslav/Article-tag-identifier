# Article-tag-identifier
Это домашка NLPOps в шаде

nlp-ops.ipynb - блокнот с обучением модели; app.py - streamlit приложение

Был обучен distilbert-base-cased на датасете https://www.kaggle.com/datasets/neelshah18/arxivdataset

Получился средний f1 score 0.7

В датасете большинство статей связаны с ML, поэтому модель предпочитает выдавать соответствующие темы)

Демо здесь: https://huggingface.co/spaces/vezhur/SHAD_NLP_ops
И нет, мне не платили за рекламу)
